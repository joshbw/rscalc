...
fn sci(expr: &str) -> assert_cmd::assert::Assert {
    rscalc().args(["-m", "sci", expr]).assert().success()
}
...
fn test_sin_30_degrees() {
    ...
    sci("sin(30)").stdout(predicate::str::starts_with("0.5"));
}

#[test]
fn test_cos_60_degrees() {
    sci("cos(60)").stdout(predicate::str::starts_with("0.5"));
}
...
#[test]
fn test_exp() {
    sci("exp(1)").stdout(predicate::str::starts_with("2.71828"));
}

#[test]
fn test_ln() {
    sci("ln(e)").stdout(predicate::str::contains("1"));
}

#[test]
fn test_log10() {
    sci("log(100)").stdout(predicate::str::contains("2"));
}

#[test]
fn test_log_base() {
    // log(8, 2) = 3
    sci("log(8, 2)").stdout(predicate::str::contains("3"));
}
...
#[test]
fn test_pow10() {
    sci("pow10(3)").stdout(predicate::str::contains("1000"));
}

#[test]
fn test_pow2() {
    sci("pow2(8)").stdout(predicate::str::contains("256"));
}

#[test]
fn test_domain_error_asin() {
    rscalc().arg("asin(2)").assert().failure();
}
